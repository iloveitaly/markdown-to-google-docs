...
async function create(
  title: string,
  folderId: string,
  options: { file?: string; credentials?: string },
) {
...
async function get(title: string, options: { credentials?: string }) {
  ...
  invariant(
    isEqual(googleDocKeys, ["content"]),
    `additional keys in google doc body ${googleDocKeys}`,
  );
...
program
  .command("get")
  .description(
    "Get the internal structure of a google doc, mostly for debugging",
  )
  .argument("<title>", "Title of the document")
  .option(
    "-c, --credentials <path>",
